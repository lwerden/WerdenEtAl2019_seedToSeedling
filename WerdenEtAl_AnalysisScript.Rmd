...
# calculate seed limitation sensu Muller-Landau et al. 2002 for species X treatment combinations @ station level 
...
# calculate source limitation sensu Muller-Landau et al. 2002 for species X treatment X site @ station level 
seedRain_sourceLimitationCalcs <- seedRain_All_byTreat_station %>% 
                       group_by(Treatment, Species, Site) %>% 
                       summarize(# total number seeds per species
                                 totalSeeds = sum(seedsSum, na.rm = TRUE)) %>%  
                                 # calculate source limitation
                                 mutate(sourceLimitation=NA) %>% 
                                 # 4 traps in each site (5 sites) 
                                 mutate(sourceLimitation=ifelse(Treatment == "C" | Treatment == "I" | Treatment == "P", exp(-(totalSeeds/4)), sourceLimitation)) %>% 
                                 # 4 traps in each site (3 sites) 
                                 mutate(sourceLimitation=ifelse(Treatment == "R", exp(-(totalSeeds/4)), sourceLimitation))
...
### Now do randomizations for seed limitation for each treatment X species combination 
The goal here is to determine if the seed limitation you observed in the field differs from a null model where seeds arrive randomly across sampling stations according to a Poisson process (Norden et al. 2009). For each species the approach will be to:  
* Assume seeds arrive randomly across the sampling stations according to a Poisson process (Norden et al. 2009) and conduct this randomization 1,000 times for each species X treatment combination. 
* Calculate randomized seed limitation for each species and compute 95% confidence intervals over the 1,000 iterations  
* Compare the randomized and observed seed limitation  
<br><br>
...
# no differences detected b/w treatments in terms of seed limitation 
Anova(modSeed_treatment_mixed, type = "II")

# color-blind friendly color palette for plotting 
CB_YlGnBu <- c("#fec44f", "#a1dab4", "#41b6c4", "#225ea8")
...
# one-way Anova testing deltaEstablishmentLimit - no differences b/w for successional status for seed limitation
...
# one-way Anova testing deltaEstablishmentLimit - no differences b/w seed limitation for seed sizes 
...
### Now do randomizations for establishment limitation for each treatment X species combination 
The goal here is to determine if the establishment limitation you observed in the field differs from a null model where seedlings establish according to a Poisson process across sampling stations (Norden et al. 2009). For each species the approach will be to:  
* Assume each seedlings establish randomly (and only at traps where seeds arrived too) across the sampling stations according to a Poisson process (Norden et al. 2009) and conduct this randomization 1,000 times for each species X treatment combination. 
* Calculate randomized establishment limitation for each species and compute 95% confidence intervals over the 1,000 iterations  
* Compare the randomized and observed establishment limitation  
<br><br>
...
# plot the deltaEstablish by treatment  
# no treatment differences so no compact letter display necessary
fig3A <- ggboxplot(establishLimitAnalysis_final,
                    x = "Treatment", 
...
establishLimitAnalysis_final_noOutlier <- establishLimitAnalysis_final_noOutlier[-c(46,56),]
...
emmeans(modEstablish_succession_mixed_noOutlier, pairwise ~ SuccessionalStage)
...
# differences between successional stage and establish limitation 
fig3B <- ggboxplot(subset(establishLimitAnalysis_final, SuccessionalStage == "Early" | SuccessionalStage == "Late"),
                    x = "SuccessionalStage", 
                    y = "deltaEstablishmentLimitation", 
                    fill = "SuccessionalStage", 
                    palette =SuccStagePalette,
...
# one-way Anova testing deltaEstablishmentLimit = no sig., no differences b/w seed size in terms of deltaEstablish
...
emmeans(modSeedSize_mixed_noOutlier, pairwise ~ seedSizeCat) # no pairwise differences... 
...
### Look at interactions between treatments and seed size or successional stage for seed and establishment limitation
* Do two-way ANOVAs on mixed effects linear regressions  
* Make Figure 2 panels A-C  
```{r}
# establishment limitation interactions

# interaction of treatment and seed size (establishment limitatation) is significant
# include as figure 3C
# result: Treatment:seedSizeCat - Chisq (3) = 20.73, p < 0.001; R 0.3324 to 0.0209, t = 4.642, p < 0.001
establish_TreatXSeedSize <- lmer(deltaEstablishmentLimitation  ~ Treatment * seedSizeCat + (1|Species), establishLimitAnalysis_final)
Anova(establish_TreatXSeedSize, type = "III")
establish_TreatXSeedSize.emm <- emmeans(establish_TreatXSeedSize, pairwise ~  seedSizeCat| Treatment)
emmip(establish_TreatXSeedSize, Treatment ~ seedSizeCat)

fig3C <- emmip(establish_TreatXSeedSize, Treatment ~ seedSizeCat)+
         theme_pubr()+
         labs(y = expression(paste("Mean ",delta["establishment"])),
                           x = "Seed Size")+
         theme(axis.title.y =  element_text(face = "bold", size = 16), 
         ...
         scale_color_manual(values=CB_YlGnBu, 
                             labels=c("NR", "AN", "P", "R"))+ 
         geom_line(aes(color = Treatment, group = Treatment), size = 1.5)+ 
         geom_point(aes(color = Treatment, group = Treatment), size = 3) 


# plot the deltaEstablish by treatment  
# no treatment differences so no compact letter display necessary

# interaction of treatment and successional status (establishment limitatation) is significant
# just include in text... consistent with figure 3C 
# result: Treatment:seedSizeCat - Chisq (2) = 24.06, p < 0.001; R 0.4114 to -0.0421, t = 4.801, p < 0.001
establish_TreatXSuccession <- lmer(deltaEstablishmentLimitation  ~ Treatment * SuccessionalStage + (1|Species),  subset(establishLimitAnalysis_final, SuccessionalStage == "Early" | SuccessionalStage == "Late"))
Anova(establish_TreatXSuccession, type = "III")
emmeans(establish_TreatXSuccession, pairwise ~ SuccessionalStage | Treatment)
emmip(establish_TreatXSuccession, Treatment ~ SuccessionalStage)

#------

# seed limitation interactions
# interaction of treatment and seed size (seed limitatation) is significant
# include as figure S2:D
# result: Treatment:seedSizeCat - Chisq (3) = 8.20, p = 0.04; IS 0.429 to 0.230, t = 2.729, p = 0.008
seed_TreatXSeedSize <- lmer(deltaSeedLimitation  ~ Treatment * seedSizeCat + (1|Species), seedLimitAnalysis_final)
Anova(seed_TreatXSeedSize, type = "III")
emmeans(seed_TreatXSeedSize, pairwise ~  seedSizeCat| Treatment)
emmip(seed_TreatXSeedSize, Treatment ~ seedSizeCat)

figS2D_interact <- emmip(seed_TreatXSeedSize, Treatment ~ seedSizeCat)+
         theme_pubr()+
         labs(y = expression(paste("Mean ",delta["seed"])),
                           x = "Seed Size")+
         theme(axis.title.y =  element_text(face = "bold", size = 16), 
               legend.title = element_text(size=14, face="bold"),
               legend.text = element_text(size = 12),
...
# interaction of treatment and successional status (seed limitatation) is not significant
# result: include in text NS result: Treatment:SuccessionalStage - Chisq (2) = 5.3633, p = 0.07
seed_TreatXSuccession <- lmer(deltaSeedLimitation  ~ Treatment * SuccessionalStage + (1|Species),  subset(seedLimitAnalysis_final, SuccessionalStage == "Early" | SuccessionalStage == "Late"))
Anova(seed_TreatXSuccession, type = "III")
emmeans(seed_TreatXSuccession, pairwise ~  SuccessionalStage| Treatment)
emmip(seed_TreatXSuccession, Treatment ~ SuccessionalStage)
```


### Compare source limitation between treatments
* Test for station-level source limitation<sub>ref</sub>among all treatments using mixed-effects one-way ANOVA (species RE)  
```{r, warning = FALSE}
# filter out for focal species 
seedRain_sourceLimitationCalcs_filter <- seedRain_sourceLimitationCalcs %>%  
                                           filter(Species %in% speciesList_seedlings_tenPercentSeedTreat$Species[speciesList_seedlings_tenPercentSeedTreat$Treatment == "C"] 
                                                  & Treatment == "C" |
                                                  Species %in% speciesList_seedlings_tenPercentSeedTreat$Species[speciesList_seedlings_tenPercentSeedTreat$Treatment == "I"] 
                                                  & Treatment == "I" |
...
##  anovas 
sourceLimitMod <- lmer(sourceLimitation~Treatment + (1|Species), seedRain_sourceLimitationCalcs_filter)
Anova(sourceLimitMod, type = "II")
sourceLim.emm <- emmeans(sourceLimitMod, pairwise ~ Treatment)
CLD(sourceLim.emm)


# make CLD for figure
obsTreat_sig <- c("a", "ab", "ab", "b")

# plot the seed limitation as just mean bars instead
plot.sourceLim.emm <- tidy(sourceLim.emm$emmeans)

fig3D <- ggbarplot(plot.sourceLim.emm,
                      x = "Treatment", 
                      y = "estimate", 
                      fill = "Treatment", 
                      palette =CB_YlGnBu,
                      xlab = "Treatment", 
                      ylab = "Source limitation",
                      legend = "none")+
...
```{r, warning=FALSE}
Figure3 <- ggarrange(fig3A, fig3B, fig3C, fig3D, labels = c("A)", "B)", "C)", "D)"), ncol = 2, nrow = 2)
Figure3
ggsave("Figure3.pdf", width = 9, height = 9, units = "in", dpi = 600, device = cairo_pdf)
```
<br><br>
...
```{r}
# save 
FigureS2 <- ggarrange(fig2A_Seed, fig2B_Seed, fig2C_Seed, figS2D_interact, labels = c("A)", "B)", "C)", "D)"), ncol = 2, nrow = 2)
FigureS2
ggsave("FigureS2.pdf", width = 9, height = 9, units = "in", dpi = 600, device = cairo_pdf)
```
...
# Make Figure S2 
FigureS1 <- mcmc_intervals(posteriorFigS2, prob = 0, prob_outer = 0.95, 
...
FigureS1
ggsave("FigureS1.pdf", width = 7, height = 6, units = "in", dpi = 600, device = cairo_pdf)
```
<br><br>
...
# by seed size
# small seeds (<5 mm)
finalModSTS_forFig2A <- brm(count_seedling ~ Treatment_Ref + meanCover + Dispersalmode + 
...
# look at summary of sampler output: no divergent transitions and acceptance probability is high 
summary(do.call(rbind, args = get_sampler_params(finalModSTS_forFig2A$fit, inc_warmup = FALSE)), digits = 2)
# summary -- all converged @ Rhat <=1.00
summary(finalModSTS_forFig2A)
# bayes R^2 = 0.51
bayes_R2(finalModSTS_forFig2A)


#---
# large seeds (>=5 mm)
finalModSTS_forFig2B <- brm(count_seedling ~ Treatment_Ref + meanCover + Dispersalmode + 
                            offset(log(seedsSum_scaled)) + (1| Species) + (1|Site), # model STS as the rate of recruits/seeds with all predictors
...
# look at summary of sampler output: no divergent transitions and acceptance probability is high 
summary(do.call(rbind, args = get_sampler_params(finalModSTS_forFig2B$fit, inc_warmup = FALSE)), digits = 2)
# summary -- all converged @ Rhat <=1.0
summary(finalModSTS_forFig2B)
# bayes R^2 = 0.53
bayes_R2(finalModSTS_forFig2B)
```
<br><br>
...
# small seeds (<5 mm)
# posterior
posterior_SmallSeed <- as.array(finalModSTS_forFig2A)
...
color_scheme_set("darkgray")
# FIGURED!
STSFig2_a <- mcmc_intervals(posterior_SmallSeed, prob = 0, prob_outer = 0.95, 
                           pars = c("Natural regeneration", "Applied nucleation", 
                                    "Plantation", "Canopy cover",
                                    "Animal dispersed"))+
...
# large seeds (>=5 mm)
# posterior
posterior_LargeSeed <- as.array(finalModSTS_forFig2B)
...
STSFig2_b <- mcmc_intervals(posterior_LargeSeed, prob = 0, prob_outer = 0.95, 
                            pars = c("Natural regeneration", "Applied nucleation", 
                                     "Plantation", "Canopy cover",
                                     "Animal dispersed"))+                           
                           labs(title="Seeds \u22655 mm")+
...
# Make Figure 1
Figure2 <- ggarrange(STSFig2_a, STSFig2_b, labels = c("A)", "B)"), ncol = 2, nrow = 1)
Figure2
ggsave("Figure2.pdf", width = 14, height = 6, units = "in", dpi = 600, device = cairo_pdf)
```
<br><br>
