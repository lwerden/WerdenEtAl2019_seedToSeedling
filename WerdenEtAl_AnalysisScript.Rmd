 ...
 * The approach of Muscarella et al. 2013 -- (traps = 0.5m^2, seedling plots = 3 m^2 = multiply by 6 to scale up) could be used to scale traps to size of vegetation transects but that would assume uniform dispersal, we are not assuming this for our manuscript but leaving this for future potential analysis  
...
The goal here is to determine if the seed limitation you observed in the field differs from a null model where seeds are Poisson distributed across the traps (Norden et al. 2019 & Muscarella et al. 2013). For each species the approach will be to:  
...
RANDO_C[[i]] <- replicate(1000, sample(plotSpeciesMatrix_C_occurrence[,i], sum(plotSpeciesMatrix_C_abundance_final[,i]), replace = TRUE))                            
}
...
RANDO_I[[i]] <- replicate(1000, sample(plotSpeciesMatrix_I_occurrence[,i], sum(plotSpeciesMatrix_I_abundance_final[,i]), replace = TRUE))                            
}
...
RANDO_P[[i]] <- replicate(1000, sample(plotSpeciesMatrix_P_occurrence[,i], sum(plotSpeciesMatrix_P_abundance_final[,i]), replace = TRUE))                            
}
...
RANDO_R[[i]] <- replicate(1000, sample(plotSpeciesMatrix_R_occurrence[,i], sum(plotSpeciesMatrix_R_abundance_final[,i]), replace = TRUE))                            
}
...
colnames(RANDO_C_df) <- c("sample", "iteration", "present", "spp") # name columns 
...
colnames(RANDO_I_df) <- c("sample", "iteration", "present", "spp") # name columns 
...
colnames(RANDO_P_df) <- c("sample", "iteration", "present", "spp") # name columns 
...
colnames(RANDO_R_df) <- c("sample", "iteration", "present", "spp") # name columns 
...
for (i in 1:length(plotSpeciesMatrix_seedling_C_occurrence[1,])){
name <- paste('item:',i,sep='')
RANDO_seedling_C[[i]] <- replicate(1000, sample(plotSpeciesMatrix_seedling_C_occurrence[,i], sum(plotSpeciesMatrix_seedling_C_abundance_final[,i]), replace = TRUE))                            
}
names(RANDO_seedling_C) <- sppNames_seedling_C                                                                        

# for islands (applied nucleation)
RANDO_seedling_I <- list()
for (i in 1:length(plotSpeciesMatrix_seedling_I_occurrence[1,])){
name <- paste('item:',i,sep='')
RANDO_seedling_I[[i]] <- replicate(1000, sample(plotSpeciesMatrix_seedling_I_occurrence[,i], sum(plotSpeciesMatrix_seedling_I_abundance_final[,i]), replace = TRUE))                            
}
names(RANDO_seedling_I) <- sppNames_seedling_I                                                                        

# for plantations
RANDO_seedling_P <- list()
for (i in 1:length(plotSpeciesMatrix_seedling_P_occurrence[1,])){
name <- paste('item:',i,sep='')
RANDO_seedling_P[[i]] <- replicate(1000, sample(plotSpeciesMatrix_seedling_P_occurrence[,i], sum(plotSpeciesMatrix_seedling_P_abundance_final[,i]), replace = TRUE))                            
}
names(RANDO_seedling_P) <- sppNames_seedling_P                                                                        

# for references
RANDO_seedling_R <- list()
for (i in 1:length(plotSpeciesMatrix_seedling_R_occurrence[1,])){
name <- paste('item:',i,sep='')
RANDO_seedling_R[[i]] <- replicate(1000, sample(plotSpeciesMatrix_seedling_R_occurrence[,i], sum(plotSpeciesMatrix_seedling_R_abundance_final[,i]), replace = TRUE))                            
}
...
colnames(RANDO_seedling_C_df) <- c("sample", "iteration", "present", "spp") # name columns 

# calculate randomized establishment limitation for each of the 1,000 iterations for species
randomEstablishLimitation_calcs_C <- RANDO_seedling_C_df %>%
                                     group_by(spp, iteration) %>%
                                     summarize(randomEstablishLimitation = 1-(sum(present)/max(sample))) 
...
observedRandomCompare_seedlingLimitation_C_final <- observedRandomCompare_establishLimitation_C[,c("Treatment", "Species", "establishLimitation", "randomEstablishLimitation_mean_C", "lower.ci", "upper.ci")]

observedRandomCompare_establishLimitation_C_final <- observedRandomCompare_seedlingLimitation_C_final %>%
                                                     mutate(deltaEstablishLimitation_C = establishLimitation-randomEstablishLimitation_mean_C)

#--------
##### for islands (applied nucleation)
RANDO_seedling_I_df <- melt(RANDO_seedling_I) # convert list of random samples to long df
colnames(RANDO_seedling_I_df) <- c("sample", "iteration", "present", "spp") # name columns 

# calculate randomized establishment limitation for each of the 1,000 iterations for species
randomEstablishLimitation_calcs_I <- RANDO_seedling_I_df %>%
                                     group_by(spp, iteration) %>%
                                     summarize(randomEstablishLimitation = 1-(sum(present)/max(sample)))
...
observedRandomCompare_seedlingLimitation_I_final <- observedRandomCompare_establishLimitation_I[,c("Treatment", "Species", "establishLimitation", "randomEstablishLimitation_mean_I", "lower.ci", "upper.ci")]

observedRandomCompare_establishLimitation_I_final <- observedRandomCompare_seedlingLimitation_I_final %>%
                                                     mutate(deltaEstablishLimitation_I = establishLimitation-randomEstablishLimitation_mean_I)


#--------
##### for plantations
RANDO_seedling_P_df <- melt(RANDO_seedling_P) # convert list of random samples to long df
colnames(RANDO_seedling_P_df) <- c("sample", "iteration", "present", "spp") # name columns 

# calculate randomized establishment limitation for each of the 1,000 iterations for species
randomEstablishLimitation_calcs_P <- RANDO_seedling_P_df %>%
                                     group_by(spp, iteration) %>%
                                     summarize(randomEstablishLimitation = 1-(sum(present)/max(sample))) 
...
observedRandomCompare_seedlingLimitation_P_final <- observedRandomCompare_establishLimitation_P[,c("Treatment", "Species", "establishLimitation", "randomEstablishLimitation_mean_P", "lower.ci", "upper.ci")]

observedRandomCompare_establishLimitation_P_final <- observedRandomCompare_seedlingLimitation_P_final %>%
                                                     mutate(deltaEstablishLimitation_P = establishLimitation-randomEstablishLimitation_mean_P)

#--------
##### for references
RANDO_seedling_R_df <- melt(RANDO_seedling_R) # convert list of random samples to long df
colnames(RANDO_seedling_R_df) <- c("sample", "iteration", "present", "spp") # name columns 

# calculate randomized establishment limitation for each of the 1,000 iterations for species
randomEstablishLimitation_calcs_R <- RANDO_seedling_R_df %>%
                                     group_by(spp, iteration) %>%
                                     summarize(randomEstablishLimitation = 1-(sum(present)/max(sample))) 
...
observedRandomCompare_seedlingLimitation_R_final <- observedRandomCompare_establishLimitation_R[,c("Treatment", "Species", "establishLimitation", "randomEstablishLimitation_mean_R", "lower.ci", "upper.ci")]

observedRandomCompare_establishLimitation_R_final <- observedRandomCompare_seedlingLimitation_R_final %>%
                                                     mutate(deltaEstablishLimitation_R = establishLimitation-randomEstablishLimitation_mean_R)
```
<br><br>
...
poissonZI_STS <- brm(count_seedling ~ Treatment_Ref + meanCover + Dispersalmode + SeedSize + 
                                           offset(log(seedsSum)) + (1| Species) + (1|Site), # STS as the rate of recruits/seeds at station 
...
finalModSTS_forFigS2 <- brm(count_seedling ~ Treatment_Ref + meanCover + Dispersalmode + SeedSize + 
                            offset(log(seedsSum)) + (1|Species) + (1|Site), # model STS as the rate of recruits/seeds at a station with all predictors
                            data = seedsAndSeedlings_forModeling_final_standard, 
...
ModSTS_onlyTreatment <- brm(count_seedling ~ Treatment_Ref + 
                            offset(log(seedsSum)) + (1|Species) + (1|Site), # model STS as the rate of recruits/seeds at a station with only treatment predictor
                            data = seedsAndSeedlings_forModeling_final_standard, 
                            family = zero_inflated_negbinomial(link = "log", link_shape = "log", link_zi = "logit"), # zero-inflated neg. binom. 
                            prior = c(set_prior("normal(0, 1e6)", class = "b"), # non-informative priors for predictors
                            set_prior("normal(0, 1e6)", class = "Intercept")), # non-informative priors for intercept
...
ModSTS_noTreatment <- brm(count_seedling ~meanCover + Dispersalmode + SeedSize + 
                            offset(log(seedsSum)) + (1|Species) + (1|Site), # model STS as the rate of recruits/seeds, only canopy cover and life-history traits as predictors
                            data = seedsAndSeedlings_forModeling_final_standard, 
                            family = zero_inflated_negbinomial(link = "log", link_shape = "log", link_zi = "logit"), # zero-inflated neg. binom. 
                            prior = c(set_prior("normal(0, 1e6)", class = "b"), # non-informative priors for predictors
                            set_prior("normal(0, 1e6)", class = "Intercept")), # non-informative priors for intercept
...
finalModSTS_forFig1A <- brm(count_seedling ~ Treatment_Ref + meanCover + Dispersalmode + 
                            offset(log(seedsSum)) + (1| Species) + (1|Site), # model STS as the rate of recruits/seeds with all predictors
