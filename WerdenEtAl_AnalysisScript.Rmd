...
# save 
FigureS3 <- ggarrange(fig2A_Seed, fig2B_Seed, fig2C_Seed, labels = c("A)", "B)", "C)"), ncol = 2, nrow = 2)
FigureS3
...
# no differences between establish limit and seed size seed size 
fig3C <- ggboxplot(establishLimitAnalysis_final,
                    x = "seedSizeCat", 
                    y = "deltaEstablishmentLimitation", 
                    fill = "seedSizeCat", 
                    palette =SSpalette,
...
fig3D <- ggbarplot(plot.obsSS.emm,
                      x = "Treatment", 
                      y = "estimate", 
                      fill = "Treatment", 
                      palette =CB_YlGnBu,
...
```{r, warning=FALSE}
Figure3 <- ggarrange(fig3A, fig3C, fig3B, fig3D, labels = c("A)", "B)", "C)", "D)"), ncol = 2, nrow = 2)
Figure3
...
### Create overall seed deposition figure for Ecol. Apps revision. 
```{r}
# make new seed size categories to match paper 
seedRain_DepRates <- seedrainWorking %>% 
                     filter(Growthform %in% c("CT", "UT")) %>% # only trees focus
                     filter(Dispersalmode %in% c("ZOO", "WIND")) %>% # only animal and wind dispersed
                     group_by(Site, Treatment, Isize, Plot, Trap, Species, Family, Growthform, Dispersalmode, SuccessionalStage, SeedSize) %>% 
                     summarize(seedsSum=sum(SR, na.rm = TRUE)) %>%
                     mutate(present = ifelse(seedsSum>0, "1", "0")) %>%
                     # add seed size category following Reid at al. 2015 - <= 5mm, >5mm 
                     mutate(seedSizeCat="NA") %>% 
                     mutate(seedSizeCat = ifelse(SeedSize == "1", "< 5 mm", seedSizeCat)) %>% 
                     mutate(seedSizeCat = ifelse(SeedSize == "2" | SeedSize == "3" | SeedSize == "4", ">= 5 mm", seedSizeCat)) %>%
                     # only use species occuring as seed in >=10% of stations on a treatment basis
                     ...
                     Species %in% speciesList_seedlings_tenPercentSeedTreat$Species[speciesList_seedlings_tenPercentSeedTreat$Treatment == "P"] & Treatment == "P" |
                     Species %in% speciesList_seedlings_tenPercentSeedTreat$Species[speciesList_seedlings_tenPercentSeedTreat$Treatment == "R"] & Treatment == "R") 
seedRain_DepRates$Species <- as.factor(seedRain_DepRates$Species)                             
# re-order seedSizeCat factor levels for plotting
seedRain_DepRates$seedSizeCat <- factor(seedRain_DepRates$seedSizeCat, levels = c("< 5 mm", ">= 5 mm"))
seedRain_DepRates$Dispersalmode <- factor(seedRain_DepRates$Dispersalmode, levels = c("ZOO", "WIND"))


# calculate deposition rates (seeds/m^-2/yr^-1) on station level for seed length categories 
# calculate deposition rates by dispersal type (seeds/m^-2/yr^-1) -- MAYBE JUST DO THIS FOR SEED SIZE -- OR FACET AND SHOW LARGER SEED SIZE BELOW??
depositionRateSummary_byPlot_mod_simp <- seedRain_DepRates %>%
                         group_by(Site, Treatment, Plot, Trap, seedSizeCat) %>%
                         summarize(totalSeeds = sum(seedsSum)) %>%
                         mutate(depositionRate = (totalSeeds/(.75*3.583)))  # area: 0.75m^2* 12 traps per site= 9m^2 per site // time = (36+7 months= 43/12 = 3.583 years)
depositionRateSummary_byPlot_mod_simp <- as.data.frame(depositionRateSummary_byPlot_mod_simp)

# drop levels
depositionRateSummary_byPlot_mod_simp <- droplevels(depositionRateSummary_byPlot_mod_simp)

# boxcos suggest log transform
#MASS::boxcox(lm(depositionRate ~ Treatment*seedSizeCat, data = depositionRateSummary_byPlot_mod_simp))

# build mixed model and test for differences b/w treatments (separated by seed size)
modSR_plot_simp <- lmer(log(depositionRate) ~ Treatment*seedSizeCat + (1|Site), depositionRateSummary_byPlot_mod_simp)
Anova(modSR_plot_simp, type = "III")
emm.simp<- emmeans(modSR_plot_simp, pairwise ~ Treatment | seedSizeCat, adjust = "tukey")
summary(emm.simp, type = "response")
CLD(emm.simp)

# mean and std.err summaries for Ms
std <- function(x) sd(x)/sqrt(length(x))

depSumForMs <- depositionRateSummary_byPlot_mod_simp %>% 
                group_by(Treatment, seedSizeCat) %>%
                dplyr::summarise(meanDep = mean(depositionRate, na.rm = T), 
                                 stdErrDep = std(depositionRate))

# plot it
# make CLD for figure
deposit_sig <- c("a", "a", "a", "a", "b", "a", "a", "b")

# combine with the upper quantile of each of the boxes to label in the same place on figure
# example: https://stackoverflow.com/questions/44712185/tukeys-post-hoc-on-ggplot-boxplot
yvalue <- depositionRateSummary_byPlot_mod_simp %>%
          group_by(Treatment, seedSizeCat) %>%
          summarize(depositionRate = quantile(depositionRate, 0.75))
final <- yvalue
final$deposit_sig <- deposit_sig

# color-blind friendly color palette for plotting 
CB_YlGnBu <- c("#ffffcc", "#a1dab4", "#41b6c4", "#225ea8")


# plot the deposition rates 
Figure1 <- ggboxplot(depositionRateSummary_byPlot_mod_simp,
                                        x = "Treatment", 
                                        y = "depositionRate", 
                                        fill = "Treatment", 
                                        palette =CB_YlGnBu,
                                        shape = "Treatment", 
                                        facet.by = c("seedSizeCat"),
                                        xlab = "Treatment", 
                                        legend = "none",
                                        ylab = expression(paste("Deposition Rate (seeds ", m^{-2}, " ", y^{-1},")")))+
                                        scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                                          labels = trans_format("log10", math_format(10^.x)))
Figure1 <- facet(Figure1, facet.by = c("seedSizeCat"),
                                     panel.labs = list(
                                     seedSizeCat = c("<5 mm", "\u22655 mm")))+
                                     theme(axis.text.x = element_text(size = 14),
                                      ...
                                      geom_text(data = final, 
                                                aes(x = Treatment, y = depositionRate, label = deposit_sig),
                                                vjust=-.45,
                                                hjust=-.28, 
                                                fontface = "bold")+
                                      scale_x_discrete(labels = c("NR", "AN", "P", "R"))
        
ggsave("Figure1.pdf", width = 8, height = 4, dpi = 600, units = "in", device=cairo_pdf)
```
